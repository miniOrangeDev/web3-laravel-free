...
if (isset($_POST['option']) && $_POST['option'] == 'save_connector_settings') {
        ...
        if(isset($_POST['discovery']) && !empty($_POST['discovery']) && !is_null($_POST['discovery'])){
            ...
            if((filter_var($discovery_endpoint, FILTER_VALIDATE_URL))){      
                ...
                if($content){
                    ...
                    $_POST['mo_web3_scopes_list'] = json_encode($scope_list);  
                    $scope = mo_web3_get_scopes($scope);
                    $_POST['mo_web3_scopes'] = json_encode($scope);
    ...
    DB::update_option('mo_web3_provider_config',json_encode($_POST));
    DB::update_option('mo_web3_message', 'Settings saved successfully!!');
}

    function mo_web3_get_scopes($scopes){
