...
use MiniOrangeWeb3\Classes\Actions\DatabaseController as DC;
...
class DB extends Controller
{
    ...
    //     self::web3_get_current_domain() = self::web3_get_current_domain();
    // }

    public static function get_option($key)
    {
        try {
            $result = LaraDB::select('select * from mo_web3_config where mo_web3_domain_name = ?', [self::web3_get_current_domain()]);
            if( count($result) == 0 ) {
                header('Location: mo_web3_create_tables');
                exit;
            } else {
                $result = $result[0];
            }
    ...
    {
        try {
            $result = LaraDB::table('mo_web3_config')->updateOrInsert([
                'mo_web3_domain_name' => self::web3_get_current_domain()
            ], [
                $key => $value
            ]);
        } catch (\Exception $e) {
    ...
    {
        try {
            $result = LaraDB::select('select * from mo_web3_admin where mo_web3_domain_name = ?', [self::web3_get_current_domain()]);
            if( count($result) == 0 ) {
                header('Location: mo_web3_create_tables');
                exit;
            } else {
    ...
    {
        try {
            LaraDB::table('mo_web3_admin')->updateOrInsert([
                'mo_web3_domain_name' => self::web3_get_current_domain()
            ], [
                'email' => $email,
                'password' => $password
    ...
    public static function get_column($table, $key)
    {
        ...
        $option = ConDB::table($table)->where(['mo_web3_domain_name' => self::web3_get_current_domain()])->first()->$key;
        return $option;
    }
    ...
    protected static function startConnection()
    {
        ...
        try {
            if (ConDB::table('mo_web3_config')->get() == NULL) {
                ConDB::table('mo_web3_config')->updateOrInsert([
                    'mo_web3_domain_name' => self::web3_get_current_domain()
                ], [
                    'mo_web3_host_name' => 'https://login.xecurify.com'
                ]);
            }
        } catch (PDOException $e) {
    ...
    private static function web3_get_current_domain()
    {
